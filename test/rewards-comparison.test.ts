...
describe("Bridge", async function () {
  ...
  it("deposit Dai and Usdc", async () => {
    ...
    // l1user deposits 30k dai and 30k usdc on L1 for l2user on L2
    ...
    expect(await usdc.balanceOf(l1user.address)).to.equal(
      l1InitialUsdcBalance - 30n * USDC_UNIT
    );
    ...
    const usdcBalance = BigNumber.from(30n * USDC_UNIT);
  ...
  it("deposit aDai and aUsdc", async () => {
    ...
    // l1user deposits 30k aDai and 30k aUsdc on L1 for l2user on L2
    ...
    blockNumberUsdc = txUsdc.blockNumber;
    expect(BigNumber.from(l1InitialAUsdcBalance - 30n * USDC_UNIT)).to.be.lte(
    ...
    const expectedaUsdcBalanceOnL2 = BigNumber.from(30n * USDC_UNIT)
  ...
  it("withdraw aDai and aUsdc to L1 user", async () => {
    ...
    expect(BigNumber.from(30n * USDC_UNIT)).to.be.lte(
    ...
    expect(BigNumber.from(27n * USDC_UNIT)).to.equal(
